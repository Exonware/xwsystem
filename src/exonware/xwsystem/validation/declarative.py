...
def _json_ser():
    """Lazy: use xwsystem JsonSerializer (no stdlib json)."""
...
class XModel(metaclass=ModelMeta):
    ...
    def model_validate_json(cls, json_data: str) -> XModel:
        """Create and validate model from JSON string (xwsystem JsonSerializer)."""
        data = _json_ser().loads(json_data)
    ...
    def model_dump_json(self, **kwargs) -> str:
        """Export model to JSON string (xwsystem JsonSerializer)."""
        data = self.model_dump(**kwargs)
        out = _json_ser().dumps(data)
