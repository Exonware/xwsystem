...
def _json_serializer():
    """Lazy: xwsystem JsonSerializer for line serialization (no stdlib json)."""
            ...
            with os.fdopen(fd, "w", encoding=encoding, newline=newline) as out_f, target.open(
                ...
                for line in in_f:
                    ...
                    if match(obj):
                        ...
                        raw = _json_serializer().dumps(updated_obj)
                        updated_line = raw.decode("utf-8") if isinstance(raw, bytes) else raw
