...
class SecureStorage:
    ...
    def store(self, key: str, value: Any, metadata: dict[str, Any] | None = None) -> None:
        ...
        """
        # Serialize value (xwsystem JsonSerializer, no stdlib json)
        from exonware.xwsystem import get_serializer, JsonSerializer
        _js = get_serializer(JsonSerializer)
        raw = _js.dumps(value)
        value_json = raw.decode("utf-8") if isinstance(raw, bytes) else raw
        # Encrypt value
    ...
    def retrieve(self, key: str) -> Any:
        ...
        # Deserialize value (xwsystem JsonSerializer)
        from exonware.xwsystem import get_serializer, JsonSerializer
        return get_serializer(JsonSerializer).loads(value_json)
