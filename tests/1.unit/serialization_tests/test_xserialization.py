...
def test_basic_detection():
    ...
    test_data = {"name": "xwsystem", "version": __version__, "formats": 24}
...
def test_content_detection():
    ...
    print(f"✅ Detected format: {serializer.get_detected_format()}")
    # Test YAML-like content (if YAML is available and content detection picks it)
    try:
        yaml_content = """
name: xwsystem
version: 0.0.1.3
...
"""
        ...
        print(f"✅ Detected format: {yaml_serializer.get_detected_format()}")
    except (ImportError, IOSerializationError):
        # Content detection may default to JSON when confidence below threshold.
        print("⚠️  YAML content detection skipped (YAML not available or detector defaulted to JSON)")
...
async def test_async_operations():
    """Test async operations (skipped if XWSerializer has no async API)."""
    print("\n🧪 Testing Async Operations...")
    test_data = {"async": "test", "concurrent": True}
    serializer = XWSerialization()
    if not hasattr(serializer, 'dumps_async'):
        pytest.skip("XWSerializer has no dumps_async (async API not implemented)")
    # Async dumps/loads
