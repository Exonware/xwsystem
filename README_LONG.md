...
### 🧭 **Indexing and search (xwsystem indexing module)**

`xwsystem` now ships a pluggable indexing module (`io/indexing`) so you can build ordered and full-text indexes on top of your data using the same core APIs:

- **In-memory and file-backed indexes** — `InMemoryIndexBackend` and `FileBackedIndexBackend` for simple key/value or document indexes that stay in RAM or persist to disk.
- **B-tree index backend** — `BTreeIndexBackend` for ordered indexes with efficient range scans (`range_scan`, `iter_from`, prefix queries).
- **Full-text index backends** — `FullTextIndexBackend` and `FileBackedFullTextIndexBackend` for tokenized term indexes with document IDs, term statistics, and on-disk persistence.
- **Shared contracts and registry** — Same registration and backend-selection patterns as the rest of xwsystem: define your index schema, pick a backend, and query it using a stable interface.

See the **Indexing** section in [docs/REF_15_API.md](docs/REF_15_API.md) for the full API, examples (building search indexes over config, logs, or entities), and performance notes.

## ⚡ Benchmarks and real-world performance

Benchmark campaigns for xwsystem live under `benchmarks/` (see [benchmarks/INDEX.md](benchmarks/INDEX.md) and [docs/REF_54_BENCH.md](docs/REF_54_BENCH.md)). The latest runs focus on **JSON**, **caching**, **serialization**, **atomic I/O**, **operations**, **data structures**, **object pool**, **validation**, **threading locks**, and the **async process fabric**.

### JSON vs other libraries (09-Feb-2026)

- **JsonSerializer** prefers **hyperjson** when installed (then hybrid msgspec+orjson, then stdlib), so with `hyperlight-hyperjson` installed it effectively matches hyperjson.
- In the benchmark campaign **“20260209-benchmark xwsystem json vs others”**:

| Payload | JsonSerializer (warm decode/encode) | orjson | msgspec | hyperjson |
|---------|-------------------------------------|--------|---------|-----------|
...
**Takeaway:** out of the box (hybrid), JsonSerializer sits in the **top tier** of JSON parsers; when you install hyperjson, it **matches hyperjson speed** while also integrating with xwsystem’s atomic I/O, caching, and indexing helpers.

### Caching vs other Python caches (10-Feb-2026)

In **“20260209-benchmark xwsystem caching vs others”** (capacity 2000, prefill 1000, 10k ops; 80% get / 20% put):
...
**Takeaway:** the default **PylruCache** wrapper (and CacheboxCache where installed) sits at **2.5–2.7M mixed ops/s**, significantly ahead of disk-backed caches and baseline Python implementations; `create_cache()` picks these defaults automatically when the underlying libraries are present.

### Serialization (MsgPack, YAML, Pickle) vs others

From **“20260210-benchmark xwsystem serialization msgpack yaml pickle”**:

- **MsgPack (small payload, warm):** MsgPackSerializer reaches **862k decode / 301k encode** ops/s vs msgpack’s **1.27M / 411k**.
- **YAML (medium payload, warm):** YamlSerializer does **15 decode / 39 encode** ops/s; PyYAML is **13 / 38** and ruamel.yaml **7 / 12**.
- **Pickle (small payload, warm):** PickleSerializer does **422k decode / 357k encode** vs stdlib pickle **1.04M / 697k**.

**Takeaway:** xwsystem serializers are **competitive with the fastest reference libraries** while giving you one uniform API across all formats (text, binary, schema-based).

### Atomic I/O vs plain writes

From **“20260210-benchmark xwsystem atomic io”** (writes/sec):

| Method           | 1 KB | 100 KB | 1 MB |
|------------------|------|--------|------|
| AtomicFileWriter | 205  | 357    | 231  |
| plain write      | 743  | 504    | 344  |

**Takeaway:** atomic, crash-safe writes cost roughly **2–3× extra latency** vs plain writes, which is acceptable for configs and critical state where you want all-or-nothing safety.

### Operations (diff/merge/patch)

From **“20260210-benchmark xwsystem operations diff merge patch”**:

- Small payloads: `diff_*` reach **35k–48k ops/s**, `merge_*` reach **59k–70k ops/s**, and `patch_apply` is **~64k ops/s**.
- Medium payloads stay around **1k–1.3k ops/s**; large nested structures are naturally slower (**60–80 ops/s**) but still usable for config/JSON-tree workflows.

**Takeaway:** structural diffs, merges, and JSON patch application are tuned for **fast config/JSON-style workloads**, and their costs are well-characterized across sizes.

### Data structures (Trie, UnionFind)

From **“20260210-benchmark xwsystem data structures trie unionfind”**:
...
**Takeaway:** **TrieNode**-based tries are far faster than you need for most path/route/indexing use-cases (nearly **900k lookups/s**), and we explicitly compare them against dict to make trade-offs clear.

### Object pool vs direct instantiation

From **“20260210-benchmark xwsystem object pool”**:
...
**Takeaway:** object pooling adds about **8× overhead** vs trivial object creation in this micro-benchmark; you should use **ObjectPool** only when object creation is expensive or when you want pooling semantics (e.g. DB connections).

### Threading locks

From **“20260210-benchmark xwsystem threading locks”**:
...
**Takeaway:** `EnhancedRLock(track_stats=False)` is the recommended **fast path** (while still adding features over stdlib); use the tracked version only when you need lock statistics or timeouts.

### Validation helpers

From **“20260210-benchmark xwsystem validation”**:
...
**Takeaway:** core validation helpers comfortably sustain **1–2.5M+ validations per second**, so you can run them on hot paths (e.g. path validation, depth checks) without worrying about them becoming bottlenecks.
